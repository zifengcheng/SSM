...
class Dream(LM):
    def __init__(
        ...
        **kwargs,
        ...
        self.sigma = sigma
        self.scale = scale
        self.preserved_tokens = preserved_tokens
        self.local_window = int(local_window)
        self.early_termination = early_termination
        self.use_suffix_soft_state = str(kwargs.get("use_suffix_soft_state", "false")).lower() in {"1", "true", "yes", "y", "on"}
        self.suffix_soft_topk = int(kwargs.get("suffix_soft_topk", 5))
        self.suffix_soft_alpha = float(kwargs.get("suffix_soft_alpha", 0.5))
        self.current_warm_start_beta = float(kwargs.get("current_warm_start_beta", 0.5))
        self.suffix_soft_non_local_only = str(kwargs.get("suffix_soft_non_local_only", "false")).lower() in {"1", "true", "yes", "y", "on"}
        if self.use_suffix_soft_state and str(self.dropout).strip().lower() == "null":
    ...
    def generate_until(self, requests: List[Instance], disable_tqdm: bool = False):
        ...
        self.model.diffusion_generate = types.MethodType(DreamGenerationMixin.diffusion_generate, self.model)
        self.model._sample = types.MethodType(DreamGenerationMixin._sample, self.model)
        self.model._sample_cache = types.MethodType(DreamGenerationMixin._sample_cache, self.model)
        self.model._sample_baseline = types.MethodType(DreamGenerationMixin._sample_baseline, self.model)
        self.model._sample_cache_baseline = types.MethodType(DreamGenerationMixin._sample_cache_baseline, self.model)
        ...
        if self.save_dir is not None:
            ...
            if self.from_scratch is False and os.path.exists(save_path):
        ...
        generated_samples = len(res)
        tokens_per_second = (self.generated_token_num / self.total_time) if self.total_time > 0 else 0.0
        avg_nfe_per_sample = (total_nfe / generated_samples) if generated_samples > 0 else 0.0
        avg_tokens_per_sample = (self.generated_token_num / generated_samples) if generated_samples > 0 else 0.0
        latency_per_sample_seconds = (self.total_time / generated_samples) if generated_samples > 0 else 0.0
        tpf = (self.generated_token_num / total_nfe) if total_nfe > 0 else 0.0

        print(f"Total number of tokens generated: {self.generated_token_num}")
        ...
        metrics = {
            ...
            "total_tokens_generated": self.generated_token_num,
            "total_time_seconds": self.total_time,
            "tokens_per_second": tokens_per_second if self.total_time > 0 else None,
            "total_nfe": total_nfe,
            "avg_nfe_per_sample": avg_nfe_per_sample,
            "avg_tokens_per_sample": avg_tokens_per_sample,
            "latency_per_sample_seconds": latency_per_sample_seconds,
            "tpf": tpf,
        }
        print(f"DREAM_METRICS_JSON: {json.dumps(metrics, ensure_ascii=False)}")
