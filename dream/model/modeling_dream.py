...
class DreamRotaryEmbedding(nn.Module):
    ...
    def _dynamic_frequency_update(self, position_ids, device):
        ...
        if seq_len > self.max_seq_len_cached:  # growth
            inv_freq, self.attention_scaling = self.rope_init_fn(
                self.config, device, seq_len, **self.rope_kwargs
            )
    ...
    @torch.no_grad()
    def forward(self, x, position_ids, seq_len, update=True):
        if seq_len is not None:
            if (
                update is False
                and (pos_sin := self.__cache.get("rope_pos_sin")) is not None
                and (pos_cos := self.__cache.get("rope_pos_cos")) is not None
                and pos_sin.shape[-2] >= seq_len
                and pos_cos.shape[-2] >= seq_len
            ):
                if pos_sin.device != x.device:
                    pos_sin = pos_sin.to(x.device)
                    self.__cache["rope_pos_sin"] = pos_sin
                if pos_cos.device != x.device:
                    pos_cos = pos_cos.to(x.device)
                    self.__cache["rope_pos_cos"] = pos_cos

                return pos_sin[:, :seq_len, :], pos_cos[:, :seq_len, :]
            
            if "dynamic" in self.rope_type:
                raise ValueError('dynamic is not supported yet')
                self._dynamic_frequency_update(position_ids, device=x.device)

            # Core RoPE block
            inv_freq_expanded = self.inv_freq[None, :, None].float().expand(position_ids.shape[0], -1, 1)
            # inv_freq_expanded = self.inv_freq[None, :, None].expand(seq_len, -1, 1)
            ...
            self.__cache["rope_pos_sin"] = sin.to(dtype=x.dtype)
            self.__cache["rope_pos_cos"] = cos.to(dtype=x.dtype)

            return self.__cache["rope_pos_cos"], self.__cache["rope_pos_sin"]
    ...
    def compute_rope(self, pos_sin: torch.Tensor, pos_cos: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        return ((t * pos_cos) + (self.rotate_half(t) * pos_sin)).to(t.dtype)


    # Copied from transformers.models.llama.modeling_llama.apply_rotary_pos_emb
    def apply_rotary_pos_emb(self, q, k, pos_cos, pos_sin, position_ids=None, unsqueeze_dim=1, block_end_index: Optional[torch.Tensor] = None, q_indices:torch.Tensor=None, k_indices:torch.Tensor=None,update=False):
        ...
        """
        pos_cos = pos_cos.unsqueeze(unsqueeze_dim)
        pos_sin = pos_sin.unsqueeze(unsqueeze_dim)
        ...
        if q_indices is None:
            ...
            if block_end_index is None:
                # print("sin: ", key_len-query_len, key_len, sin[:, :, key_len - query_len : key_len, :])
                q_embed = (q * pos_cos[:, :, key_len - query_len : key_len, :]) + (self.rotate_half(q) * pos_sin[:, :, key_len - query_len : key_len, :])
            else:
                q_embed = (q * pos_cos[:, :, block_end_index.item() - query_len : block_end_index.item(), :]) + (self.rotate_half(q) * pos_sin[:, :, block_end_index.item() - query_len : block_end_index.item(), :])
            k_embed = (k * pos_cos) + (self.rotate_half(k) * pos_sin)
...
class DreamAttention(nn.Module):
    ...
    def __init__(self, config: DreamConfig, layer_idx: Optional[int] = None, cache: Optional[BufferCache] = None):
        super().__init__()
        self.config = config
        ...
        self.rotary_emb = DreamRotaryEmbedding(config=self.config, cache = self.__cache)
    ...
    def forward(
        ...
        update_rope: Optional[bool] = True,
        ori_seq_len: Optional[int] = None
        ...
        if position_embeddings is None:
            ...
            cos, sin = self.rotary_emb(value_states, position_ids, ori_seq_len, update_rope)
        else:
            cos, sin = position_embeddings
        query_states, key_states = self.rotary_emb.apply_rotary_pos_emb(query_states, key_states, cos, sin, q_indices=q_indices, k_indices=k_indices, update=update_rope)
...
class DreamSdpaAttention(DreamAttention):
    ...
    def forward(
        ...
        if dual_cache:
            query_states, key_states = self.rotary_emb.apply_rotary_pos_emb(query_states, key_states, cos, sin, block_end_index=replace_indices.max()+1, q_indices=q_indices, k_indices=k_indices, update=update_rope)
        else:
            query_states, key_states = self.rotary_emb.apply_rotary_pos_emb(query_states, key_states, cos, sin, q_indices=q_indices, k_indices=k_indices, update=update_rope)
...
class DreamDecoderLayer(nn.Module):
    def __init__(self, config: DreamConfig, layer_idx: int, cache: BufferCache):
        super().__init__()
        self.hidden_size = config.hidden_size
        ...
        self.self_attn = DreamSdpaAttention(config, layer_idx, self.__cache)
...
class DreamBaseModel(DreamPreTrainedModel):
    ...
    def __init__(self, config: DreamConfig):
        ...
        self.layers = nn.ModuleList(
            [DreamDecoderLayer(config, layer_idx, self.__cache) for layer_idx in range(config.num_hidden_layers)]
        )
        self._attn_implementation = config._attn_implementation
        self.norm = DreamRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.rotary_emb = DreamRotaryEmbedding(config=config, cache=self.__cache)
    ...
    def forward(
        ...
        if ori_seq_len is not None:
            if not dual_cache:
                position_ids = torch.arange(past_seen_tokens + ori_seq_len, device=inputs_embeds.device).unsqueeze(0)
            else:
                position_ids = torch.arange(ori_seq_len, device=inputs_embeds.device).unsqueeze(0)
        else:
            # Original Fast-dLLM Implementation
            if not dual_cache:
            ...
            # print("position_ids:", position_ids.shape, ori_seq_len)
        ...
        position_embeddings = self.rotary_emb(hidden_states, position_ids, ori_seq_len, update_rope)
        # decoder layers
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            ...
            update_rope = update_rope & (layer_idx == 0)
            if self.gradient_checkpointing and self.training:
