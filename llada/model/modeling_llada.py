...
class LLaDABlock(nn.Module):
    ...
    def attention(
        ...
        # if past_key_values_last_step is not None:
        #     # old_q_shape = q.shape
        #     # old_k_shape = k.shape
        #     # old_v_shape = v.shape        
    ...
    def build(cls, layer_id: int, config: ModelConfig, cache: BufferCache) -> LLaDABlock:
        if config.block_type == BlockType.sequential:
            assert 0, "LLaDASequentialBlock is not implemented in this version. Please use LLaDALlamaBlock instead."
...
class LLaDAModel(nn.Module):
    def __init__(self, config: ModelConfig, init_params: bool = True):
        ...
        if self.config.block_group_size > 1:
            raise ValueError("Block group size greater than 1 is not supported")
            # block_groups = [
    ...
    def forward(
        ...
        assert self.config.block_group_size == 1, "multiple block groups are not supported"
        assert len(self.transformer.blocks) > 0, "no transformer blocks found"
